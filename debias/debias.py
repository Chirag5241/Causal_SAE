...
def get_debiasing_projection(classifier_class, cls_params: Dict, num_classifiers: int, input_dim: int,
                             ...
                             Y_dev: np.ndarray, best_iter_diff=0.01, summary_writer=None, pth=None, prop=None) \
    ...
    do_tqdm = False # marc
    pbar = tqdm(range(num_classifiers)) if do_tqdm else range(num_classifiers)
    for i in pbar:
        clf = classifier.SKlearnClassifier(classifier_class(**cls_params))
        acc = clf.train_network(X_train_cp, Y_train, X_dev_cp, Y_dev)
        desc = "iteration: {}, accuracy: {}".format(i, acc)
        if do_tqdm: pbar.set_description(desc)
        else: print(desc)
        ...
        with open(pth + '/clf_' + str(i) + '.pkl', 'wb') as f:
            pickle.dump(clf, f)
        ...
        if is_autoregressive:
            ...
            # Save inlp embeddings
            test_embeddings_attack = torch.tensor(X_dev_cp.dot(P)).type(torch.float32)
            torch.save(test_embeddings_attack, pth +
                       '/vectors_' + str(i+1) + '.pt')
...
def get_debiasing_projection_by_cls(classifier_class, cls_params: Dict, num_classifiers: int, input_dim: int,
    ...
    do_tqdm =False # marc
    itterr = range(len(all_labels))
    pbar = tqdm(itterr, ncols=600) if do_tqdm else itterr
    for i in pbar:
        pbar_inner = tqdm(range(60), ncols=600) if do_tqdm else range(60)
        for j in pbar_inner:
            clf = classifier.SKlearnClassifier(classifier_class(**cls_params))
            dropout_scale = 1. / (1 - dropout_rate + 1e-6)
            dropout_mask = (np.random.rand(*X_train.shape) <
                            (1 - dropout_rate)).astype(float) * dropout_scale
