...
from debias.debias import get_debiasing_projection, debias_by_specific_directions, get_pls_projection
from debiased_finetuning.utils import define_network
...
def get_top_k_lm_predictions_gpu(tokenizer, w, b, x, y, projection: np.ndarray = None,
                                 k=100, device: str = 'cpu', concat_mask=False):
    network = define_network(
        w, b, projection_mat=projection, device=device, concat_mask=concat_mask)
...
def learn_cls(x_train, y_train, x_dev, y_dev):
    clf = SGDClassifier(warm_start=True, loss='log', n_jobs=-1,
                        max_iter=10000, random_state=0, early_stopping=True)
...
def get_projection_matrix(num_clfs, x_train, y_train, x_dev, y_dev,
                          majority_acc, max_iter=500, summary_writer=None, pth=None, property=None):
    clf = SGDClassifier
    params = {'warm_start': True, 'loss': 'log_loss', 'n_jobs': -1, 'max_iter': max_iter, 'random_state': 0,
              'early_stopping': True}
    dim = x_train.shape[-1]
    if len(x_train.shape) == 4:
        x_train = x_train.squeeze(2).squeeze(1)
        x_dev = x_dev.squeeze(2).squeeze(1)
    ...
    P, _, _, all_projections, best_projection = get_debiasing_projection(clf, params, num_clfs, dim,
                                                                         ...
                                                                         summary_writer=summary_writer, pth=pth, prop=property)
...
def most_probable_label(words, labels):
    ...
    for word, label_list in words_labels.items():
        most_probable_label_per_word[word] = Counter(
            label_list).most_common(1)[0][0]
...
def convert_words2labels(words, probable_labels, label2ind, most_common_label):
    ...
    for word in words:
        labels_freqeuency[label2ind[probable_labels.get(
            word, most_common_label)]] += 1
...
def calc_entropy(x, y, y_labels, probable_labels, tokenizer, out_embed, bias, k, device):
    ...
    entropy_score = torch.distributions.Categorical(
        logits=all_dists).entropy().mean().cpu().numpy()
